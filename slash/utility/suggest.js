...
const { ChatInputCommand, ModalBuilder, TextInputBuilder, TextInputStyle, ActionRowBuilder, EmbedBuilder, ButtonBuilder, ButtonStyle } = require('discord.js');
...
module.exports = {
  ...
  async execute(interaction) {
    const guildConfiguration = await GuildConfiguration.findOne({ guildId: interaction.guilId });

    if (!guildConfiguration || !guildConfiguration.suggestionChannelIds.length) {
      await interaction.reply('This server has not been configured to use suggestions yet. Ask an admin to configure it.');
      return;
    }
    ...
    const modal = new ModalBuilder()
      ...
      .setCustomId(`suggestions-${interaction.guild.id}`);

    const textInput = new TextInputBuilder()
      .setCustomId('suggestion-input')
      .setLable('What would you like to suggest?')
    ...
    const filter = (i) => i.customId === `suggestions-${interaction.guild.id}`
    ...
    try {
      ...
      modalInteraction.editReply(
        'Failed to create a suggestion message in this channel. I may not have enough permissions.'
      );
      return;
    }
